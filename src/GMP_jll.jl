...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import GMP_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        override_dir = joinpath(Pkg.devdir(), "GMP_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"GMP", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "GMP_jll", "src", "GMP_jll.jl"))
        @info("GMP_ll dev'ed out to /depot/dev/GMP_jll with pre-populated override directory")
    end
end
...
if best_platform === nothing
    @debug("Unable to load GMP; unsupported platform $(triplet(platform_key_abi()))")
else
